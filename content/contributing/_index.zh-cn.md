...
- `content/contributors/` - 贡献者页面（社区成员信息，自动更新）
- `content/authors/` - 作者页面（文章作者信息）
- `content/contributing/` - 贡献指南（本页面）
- `content/changelog/` - 更新记录（网站内容变更日志）
- `content/posts/` - 新闻文章和教程目录
- `content/categories/` - 文章分类定义

每个栏目包含简体中文（`_index.zh-cn.md`）和传统中文（`_index.zh-tw.md`）版本。
