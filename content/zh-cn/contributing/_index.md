...
- **为 gentoo-zh Overlay 贡献**（软件包 / ebuild）——社区主线，也是[贡献者墙](/contributors/)的来源（脚本每月抓取 [gentoo-zh/overlay](https://github.com/gentoo-zh/overlay) 提交 5 次以上者）。详见下方「为 gentoo-zh Overlay 贡献」。
...
{{< callout type="info" >}}
overlay 仓库已迁移到组织仓库 [gentoo-zh/overlay](https://github.com/gentoo-zh/overlay)，本页链接均已更新。旧的 `microcai/gentoo-zh` 个人仓库会 301 到新仓库，fork、本地 remote 建议在方便时更新到新 URL，详见[公告与执行方案](/posts/2026-07-02-gentoo-zh-repo-migration/)。
{{< /callout >}}
...
仓库用 [nvchecker](https://github.com/lilydjwg/nvchecker) 每天自动比对各包的上游版本（配置在 `.github/workflows/overlay.toml`），有新版本就自动开/更新对应的 [GitHub issue](https://github.com/gentoo-zh/overlay/issues)——**不知道从哪下手，挑一个版本更新（bump）issue 来做最省心**。新增包时，记得也在 `overlay.toml` 中加一条 nvchecker 规则（写明上游版本来源），让它一并纳入版本追踪。
