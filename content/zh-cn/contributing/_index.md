...
- **为 gentoo-zh Overlay 贡献**（软件包 / ebuild）——社区主线，也是[贡献者墙](/contributors/)的来源（脚本每月抓取 [Gentoo-zh/gentoo-zh](https://github.com/Gentoo-zh/gentoo-zh) 提交 5 次以上者）。详见下方「为 gentoo-zh Overlay 贡献」。
...
gentoo-zh 是一个 `masters = gentoo` 的 Gentoo overlay（叠加在官方 Portage 树之上），收录 450 多个包，源码在 [Gentoo-zh/gentoo-zh](https://github.com/Gentoo-zh/gentoo-zh)。新增或更新 ebuild、修 bug、跟进新版本，都通过 GitHub Pull Request 提交；发现问题也欢迎提 [issue](https://github.com/Gentoo-zh/gentoo-zh/issues)。

{{< callout type="info" >}}
overlay 仓库已迁移到组织仓库 [Gentoo-zh/gentoo-zh](https://github.com/Gentoo-zh/gentoo-zh)，本页链接均已更新。旧的 `microcai/gentoo-zh` 个人仓库会 301 到新仓库，fork、本地 remote 建议在方便时更新到新 URL，详见[公告与执行方案](/posts/2026-07-02-gentoo-zh-repo-migration/)。
{{< /callout >}}
...
到 [GitHub](https://github.com/Gentoo-zh/gentoo-zh) fork 仓库、clone 你的 fork、新建分支；本地启用 overlay 方便测试（见 [Overlay 页](/overlay/)）。
...
仓库用 [nvchecker](https://github.com/lilydjwg/nvchecker) 每天自动比对各包的上游版本（配置在 `.github/workflows/overlay.toml`），有新版本就自动开/更新对应的 [GitHub issue](https://github.com/Gentoo-zh/gentoo-zh/issues)——**不知道从哪下手，挑一个版本更新（bump）issue 来做最省心**。新增包时，记得也在 `overlay.toml` 中加一条 nvchecker 规则（写明上游版本来源），让它一并纳入版本追踪。
...
- 本仓库 [README](https://github.com/Gentoo-zh/gentoo-zh#readme) 与[依赖关系表](https://github.com/Gentoo-zh/gentoo-zh/blob/deps-table/relation.md)
...
内容按语言分目录存放，简体中文在 `content/zh-cn/`，正体中文在 `content/zh-tw/`，两者目录结构完全一致：
...
> 同一篇内容的简体版放在 `content/zh-cn/...`、正体版放在对应的 `content/zh-tw/...`，文件名都是 `index.md` / `_index.md`（**不再**使用 `index.zh-cn.md` 这种语言后缀）。
...
- 默认语言为简体中文，位于站点根路径 `/`；正体中文位于 `/zh-tw/`
...
错别字、过时信息、使用技巧、缺失的正体中文翻译，看到了都欢迎随手修正。模板、样式、性能、功能等技术层面，也欢迎提改进。

> **贡献者列表（`content/*/contributors/`）由脚本自动维护**，抓取 [gentoo-zh Overlay](https://github.com/Gentoo-zh/gentoo-zh) 中提交 5 次以上者，显示提交次数并按提交量排序，每月自动更新（`scripts/update-contributors.py` + GitHub Actions）。**请勿手动编辑该目录**；首页贡献者展示也随之自动更新。
