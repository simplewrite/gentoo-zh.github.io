...
為了方便那些不願使用社群即時交流方式，或者不想在群裡刷屏、想把問題說清楚的人；也方便一些平時被各個平臺隔開、湊不到一塊兒的中文使用者。
...
├── 磁碟加密與自動解鎖
│   ├── LUKS2 全盤加密（AES-256-XTS + Argon2id）
│   ├── 網路繫結自動解鎖（Clevis + Tang · McCallum-Relyea 協議）
│   │   ├── 開機自動重建金鑰解盤，金鑰不落任何磁碟、不依賴 TPM
│   │   └── 多節點 Tang 互信冗餘，任一對端線上即可自動解鎖
│   └── dropbear-initramfs（SSH 遠端手動解鎖 · 兜底）
├── 高可用與自愈
│   ├── NVMe 塊儲存 · CEPH 三副本
│   ├── 機密計算 VM 不能熱遷移，宿主維護會觸發重啟（曾因此手動解密卡過一次）
│   └── 自建自愈閉環：自動解鎖 + 跨節點看門狗 → 數分鐘內自動開機、自動解密、恢復，全程無人
├── 網路防護
...
│   ├── Telegram 即時告警 · 公開狀態頁 status.gentoozh.org（簡 / 繁 / 英）
...
多說兩句這次新加的「自動解密」和監控。全盤 LUKS 以前得有人手動 SSH 進去輸密碼解鎖——機密計算的機器一旦被宿主維護重啟，就只能乾等著人來解密（我們確實為此卡過一次）。現在換成 **Clevis + Tang** 的網路繫結解鎖：開機時向多臺互信節點重新算出解密金鑰（McCallum-Relyea 協議），金鑰不落盤、也不塞進 TPM，任一節點線上就能自動開鎖；配上跨節點看門狗，做到「宿主維護重啟 → 自動開機 + 自動解密 + 恢復服務」全程無人。dropbear 遠端手動解鎖仍保留作兜底。

監控也從單機撥測升級成**兩套異地 Prometheus/Alertmanager 互相盯著、叢集去重**——任一套（連同它所在的機房）掛了，另一套照樣發告警，不會出現「監控和服務一起靜默」。主機、四個網站的可用性、SSL 到期都在盯，告警即時進 Telegram，狀態同步公開到 [status.gentoozh.org](https://status.gentoozh.org)。
...
版塊和橋接後面還會接著調，用著順手點；機器、高可用和備份這些，等人多了再慢慢補。
