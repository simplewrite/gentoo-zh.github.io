...
description: "在 M1/M2 MacBook 上裝 Gentoo 的完整流程，基於 Asahi Linux：Live USB、分割區、核心到桌面都有。"
---
...
不要修改現有的 APFS 容器、EFI 分割區或 Recovery 分割區，只在 Asahi 安裝程式預留的空間中操作。
...
使用 `cfdisk /dev/nvme0n1` 在空閒空間建立 Linux filesystem 型別分割區。
...
獲取分割區 UUID（按你實際的裝置路徑調整）：

```bash
# 根分割區（非加密直接用裝置；加密用解密後的 mapper 裝置）
blkid /dev/nvme0n1p6                    # 非加密
blkid /dev/mapper/gentoo-root           # 加密

# EFI 分割區（Asahi 安裝時設的卷標 "EFI - GENTO"）
blkid --label "EFI - GENTO"
...
首次建置桌面環境約需 2–4 小時，建議 `--jobs 3` 或更少避免 OOM。
...
macOS 更新會附帶韌體更新，建議保留 macOS 分割區以便獲取最新韌體。
