...
- **為 gentoo-zh Overlay 貢獻**（軟體套件 / ebuild）——社群主線，也是[貢獻者牆](/contributors/)的來源（指令碼每月抓取 [gentoo-zh/overlay](https://github.com/gentoo-zh/overlay) 提交 5 次以上者）。詳見下方「為 gentoo-zh Overlay 貢獻」。
...
倉庫用 [nvchecker](https://github.com/lilydjwg/nvchecker) 每天自動比對各包的上游版本（配置在 `.github/workflows/overlay.toml`），有新版本就自動開/更新對應的 [GitHub issue](https://github.com/gentoo-zh/overlay/issues)——**不知道從哪下手，挑一個版本更新（bump）issue 來做最省事**。新增包時，記得也在 `overlay.toml` 中加一條 nvchecker 規則（寫明上游版本來源），讓它一併納入版本追蹤。
