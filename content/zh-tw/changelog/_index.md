...
- 新增文章 **[如何參與 Gentoo Wiki 的翻譯工作](/posts/2026-06-30-gentoo-wiki-translation/)**：面向中文譯者的翻譯入門，涵蓋賬號與權限申請、翻譯介面用法、翻譯規範與中英文排版約定，以及 Gentoo 理事會 AI 政策對翻譯的具體影響（作者 YangMame）
...
- 新增公告 **[中文社群近期更新：Live ISO、官網、下載站與測速](/posts/2026-06-09-live-iso-improvements/)**：定製 KDE Live ISO、Calamares 安裝器、自動構建流水線、官網 Hextra 遷移與英文國際化、下載站與測速站的整體進展
...
- 新增轉載文章 **[Gentoo Linux with ZFS](/posts/2026-06-18-gentoo-linux-with-zfs/)**（原作者 [Locez](https://github.com/locez)，經授權按 CC BY-NC-SA 4.0 轉載）：在雙 NVMe 映象上安裝 ZFS 根 + ZFS 原生加密的實錄；本站補充了 SLOG 配置勘誤、分割槽要點，以及各安裝步驟對應的官方中文手冊連結
...
- 下載站上雲：Live ISO 下載遷到 **Cloudflare R2**（[r2.gentoozh.org](https://r2.gentoozh.org/)，零出口流量、全球邊緣），落地頁 [mirror.gentoozh.org](https://mirror.gentoozh.org/) 改成 **Cloudflare Worker**（邊緣即時讀 R2，列出最新映象 + 全部歷史版本）；測速改用 [Cloudflare 官方測速](https://speed.cloudflare.com/)；自建的美國下載 / 測速伺服器隨之下線
- 公共頁面新增英文（English）國際化：關於、下載、映象列表、貢獻指南等公共頁面現在可在簡體 / 繁體 / 英文之間切換，主要是方便用 gentoo-zh overlay 的海外使用者。需要說明：**技術文章不一定都有英文**，目前只做了公共頁面；英文部分借翻譯軟體協助生成、由至今能找到的最好最貴的模型 Claude Fable 5（ultracode）負責 review 和最佳化，難免有錯漏，歡迎在 [GitHub](https://github.com/Gentoo-zh/gentoo-zh.github.io) 指正
...
- 下載站接入 mirror.gentoozh.org，替代已下線的 iso.gig-os.org；下載 / 映象列表 / Overlay 三頁重寫，Live ISO 登入憑據支援點選複製
- 映象同步源說明：Gentoo 官方自 2025-10-30 起停止為第三方倉庫（含 gentoo-zh）提供快取映象，overlay 已改為直連 GitHub 上游同步；之前新增過的使用者需更新同步源，詳見[公告](/posts/2025-10-07-thirdparty-repo-mirror-removal/)
