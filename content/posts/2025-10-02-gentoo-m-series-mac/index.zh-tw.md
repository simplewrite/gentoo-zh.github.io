...
categories: ["tutorial"]
authors: ["zakkaus"]
summary: "Apple Silicon Mac (M1/M2) Gentoo Linux 安裝全攻略，涵蓋 Asahi Linux 引導、GPU 驅動、桌面環境配置。注意：M3/M4/M5 暫不支援。"
description: "2025 年最新 Apple Silicon Mac (M1/M2) Gentoo Linux 安裝指南，基於 Asahi Linux 專案，套件含 Live USB 製作、分割槽、核心安裝及桌面環境配置。M3/M4/M5 晶片暫不支援。"
---
![Gentoo on Apple Silicon Mac](gentoo-asahi-mac.webp)
**簡介**
本指南將引導你在 Apple Silicon Mac（**M1/M2 系列**）上安裝原生 ARM64 Gentoo Linux。
**⚠️ 重要提示：硬體相容性**
**支援的裝置**：M1 和 M2 系列 MacBook（Pro、Air、Mac Mini 等）
**暫不支援**：M3、M4、M5 系列晶片目前暫不支援，請等待 Asahi Linux 專案更新。
...
**目標平臺**：Apple Silicon Mac（**M1/M2 系列**）ARM64 架構。本指南使用 Asahi Linux 載入程式進行初始設定，然後轉換為完整的 Gentoo 環境。
---
...
**官方簡化**
現在可使用 [asahi-gentoosupport 自動化指令碼](https://github.com/chadmed/asahi-gentoosupport) 完成大部分配置！
---
...
*   Apple Silicon Mac（**僅 M1/M2 系列晶片，M3/M4/M5 暫不支援**）
...
```
\> **安全提示**
建議先前往 <https://alx.sh> 檢視指令碼內容，確認安全後再執行。

## 1.2 跟隨安裝程式步驟
安裝程式會引導你：
1. **選擇動作**：輸入 `r` (Resize an existing partition to make space for a new OS)
2. **選擇分割槽空間**：決定分配給 Linux 的空間（建議至少 80 GB）
   - 可使用百分比（如 `50%`）或絕對大小（如 `120GB`）

> **提示**：建議保留 macOS 分割槽，以便日後更新韌體。
...
5. 你會看到 macOS Recovery 畫面：
   - 若要求「Select a volume to recover」，選擇你的 macOS 音量並點選 Next

   - 輸入 macOS 使用者密碼（FileVault 使用者）

6. 依照螢幕指示完成設定
**故障排除**
若遇到啟動迴圈或要求重新安裝 macOS，請按住電源鍵完全關機，然後從步驟 1 重新開始。可選擇 macOS 開機，執行 `curl https://alx.sh | sh` 並選擇 `p` 選項重試。
---
...
```
> **提示**：Apple Silicon 的 Wi-Fi 已套件含在核心中，應可正常運作。若不穩定，嘗試連線 2.4 GHz 網路。
**（可選）SSH 遠端操作**：
```bash
passwd                     
# 設定 root 密碼
...
## 3. 分割槽與檔案系統設定 {#step-3-partition}

## 3.1 識別磁碟與分割槽
**重要警告**
**不要修改現有的 APFS 容器、EFI 分割槽或 Recovery 分割槽！** 只能在 Asahi 安裝程式預留的空間中操作。
檢視分割槽結構：
```bash
lsblk
...
```
**建議**
使用 `cfdisk` 進行分割槽，它理解 Apple 分割槽型別並會保護系統分割槽。
...
**建立使用者與設定密碼**：
```bash
# 建立使用者（替換 <使用者名稱稱> 為你的使用者名稱）
useradd -m -G wheel,audio,video,usb,input <使用者名稱稱>
# 設定使用者密碼
passwd <使用者名稱稱>
# 設定 root 密碼
passwd root
...
- 安裝 asahi-meta（套件含核心、韌體、m1n1、U-Boot）
...
sync-type = git
sync-uri = https://ftp.twaren.net/git/Gentoo/gentoo-portage.git
auto-sync = yes
sync-depth = 1
EOF
...
```
> **提示**：如果 `etc-update` 出現配置檔案衝突，選擇 `-3` 進行自動合併。
套件說明：

- `rust-bin`：編譯 Asahi 核心元件需要（必須先安裝）
- `asahi-meta`：套件含 m1n1、asahi-fwupdate、U-Boot 等工具

- `virtual/dist-kernel:asahi`：Asahi 特製核心（套件含未上游的補丁）
...
獲取 UUID：
```bash
blkid $(blkid --label root)       
# 根分割槽（或 /dev/mapper/gentoo-root）
blkid $(blkid --label "EFI - GENTO")     
# boot 分割槽
```
編輯 `/etc/fstab`：
```bash
...
- `rd.luks.uuid=<UUID>`：LUKS 加密分割槽的 UUID（使用 `blkid /dev/nvme0n1p6` 獲取）
...
- `rootfstype=btrfs`：根檔案系統型別（如果使用 ext4 改為 `ext4`）
...
```
**重要**
每次更新核心後，也需要重新執行此指令產生新的 initramfs！
**步驟 8：更新 GRUB 配置**
```bash
grub-mkconfig -o /boot/grub/grub.cfg
# 驗證 initramfs 被正確引用
...
## 8.1 定期更新流程
```bash
# 更新 Portage 樹（套件含 Asahi overlay）
emerge --sync
# 或手動同步 Asahi overlay：
emaint -r asahi sync
# 更新所有套件
...
每次核心更新後必須執行！
```bash
# 更新 m1n1 Stage 2（套件含 devicetree）
update-m1n1
# 重新產生 GRUB 配置
grub-mkconfig -o /boot/grub/grub.cfg
```
**為什麼？** m1n1 Stage 2 套件含 devicetree blobs，核心需要它來識別硬體。不更新可能導致無法啟動或功能缺失。
**自動化**
`sys-apps/asahi-scripts` 提供 installkernel hook 自動執行這些步驟。

## 8.3 更新韌體
macOS 系統更新時會套件含韌體更新。**建議保留 macOS 分割槽**以便獲取最新韌體。
---
...
3. 重新產生 GRUB 配置：`grub-mkconfig -o /boot/grub/grub.cfg`
...
*   [User:Jared/Gentoo On An M1 Mac](https://wiki.gentoo.org/wiki/User:Jared/Gentoo_On_An_M1_Mac) - 社群成員的安裝指南
---
