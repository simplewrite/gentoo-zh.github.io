...
Gentoo 釋出安全公告 (GLSA) 來通知使用者潛在的安全漏洞。
...
> 提示：`emerge --search` 速度較慢，推薦使用 `eix` 進行毫秒級搜尋。
...
詳細配置請參考 **Section 15. LTO 與 Clang 編譯最佳化**。
...
<details>
<summary>LTO 優缺點詳細分析（點選展開）</summary>
...
# -Werror=lto-type-mismatch: LTO 型別不匹配
...
建立 `/etc/portage/package.env/no-lto` 檔案（套件含已知問題套件）：
...
如果遇到其他 LTO 相關的連結錯誤，請先嚐試停用該套件的 LTO。也可以檢視 [Gentoo Bugzilla](https://bugs.gentoo.org) 搜尋是否已有相關報告（搜尋"軟體套件名 lto"）。如果是新問題，歡迎提交 bug 報告幫助改進 Gentoo。
...
本節面向希望深入掌控核心編譯的高階使用者，套件括使用 LLVM/Clang 編譯、啟用 LTO 最佳化、自動化配置等。
...
| 英文選項 | 中文說明 | 關鍵配置 |
| :--- | :--- | :--- |
| **General setup** | 通用設定 | 本地主機名、Systemd/OpenRC 支援 |
| **Processor type and features** | 處理器型別與特性 | CPU 型號選擇、微碼載入 |
...
配置完成後，務必檢查 `/boot/initramfs-*.img` 是否套件含 RAID 模組：
